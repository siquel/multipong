...
namespace common
{
    ...
    {
        JKN_ASSERT(count > 0, "count must be more than 0");
        JKN_ASSERT(count <= 32, "count must be less than 32");
        JKN_ASSERT(m_bitsRead + count <= m_numBits, "Out of bounds");

        m_bitsRead += count;

        JKN_ASSERT(m_scratchBits >= 0 && m_scratchBits <= 64, "Overflow");

        if (m_scratchBits < count)
        {
            JKN_ASSERT(m_wordIndex < m_numWords, "Overflow");
        ...
        JKN_ASSERT(m_scratchBits >= count, "fucked up");
    ...
    bool BitReader::readAlign()
    {
        ...
        if (remainderBits != 0)
        {
            ...
            JKN_ASSERT(m_bitsRead % 8 == 0, "Reader not aligned");
    ...
    {
        JKN_ASSERT(getAlignBits() == 0, "Reader not aligned to byte boundary");
        JKN_ASSERT(m_bitsRead + bytes * 8 <= m_numBits, "Out of bounds");
        JKN_ASSERT(0
            || (m_bitsRead % 32) == 0
            || (m_bitsRead % 32) == 8
            || (m_bitsRead % 32) == 16
        ...
        JKN_ASSERT(getAlignBits() == 0, "Reader not aligned");
        ...
        if (numWords > 0)
        {
            JKN_ASSERT((m_bitsRead % 32) == 0, "Reader not aligned");
            // aligned read
        ...
        JKN_ASSERT(tailBytes >= 0 && tailBytes < 4, "Bytes are fucked up");
        ...
        JKN_ASSERT(getAlignBits() == 0, "Reader not aligned");
        JKN_ASSERT(headBytes + numWords * 4 + tailBytes == bytes, "Read truncated");
    }
