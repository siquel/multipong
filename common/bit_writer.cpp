...
namespace common
{
    ...
    {
        JKN_ASSERT(bits > 0, "Bits to write must be more than 0");
        JKN_ASSERT(bits <= 32, "Bits to write must be less than 32");
        JKN_ASSERT(m_bitsWritten + bits <= m_numBits,
        ...
        */
        ...
        if (m_scratchBits >= 32)
        {
            JKN_ASSERT(m_wordIndex < m_numWords, "Out of space");
    ...
    void BitWriter::writeAlign()
    {
        ...
        if (remainderBits != 0)
        {
            ...
            JKN_ASSERT((m_bitsWritten % 8) == 0, "Write not aligned");
        }
    }
    ...
    {
        JKN_ASSERT(getAlignBits() == 0, "Writer must be aligned, (align bits = %d)", getAlignBits());
        JKN_ASSERT(m_bitsWritten + (bytes * 8) <= m_numBits,
        ...
        */
        ...
        JKN_ASSERT(getAlignBits() == 0, "Writer not aligned");
        ...
        if (numWords > 0)
        {
            JKN_ASSERT((m_bitsWritten % 32) == 0, "Writer not aligned");
        ...
        JKN_ASSERT(headbytes + numWords * 4 + tailBytes == bytes,
